...
namespace Modbus_Poll_CS
{
    class modbus
    {
        ...
        public bool SendFc16(byte address, ushort start, ushort registers, byte[] values)
        {
            //Ensure port is open:
            if (sp.IsOpen)
            {
                ...
                {
                    message[7 + 2 * i] = (byte)(values[2 * i + 1]);
                    message[8 + 2 * i] = (byte)(values[2 * i]);
                }
        ...
        public bool SendFc3(byte address, ushort start, ushort registers, ref ushort[] values)
        {
            ...
            if (sp.IsOpen)
            {
                ...
                if (CheckResponse(response))
                {
                    ...
                    {
                        values[start + i] = response[2 * i + 4];
                        values[start + i] <<= 8;
                        values[start + i] += response[2 * i + 3];
                    }
